...
func (self *SmallbankHandler) Apply(request *processor_pb2.TpProcessRequest, context *processor.Context) error {
	...
	logger.Debugf("smallbank signature",
		request.Signature)
	logger.Infof("TP working")
	logger.Infof("Your Payload- ", payload)
...
func unpackPayload(payloadRaw []byte) (*payloadData.Data, error) {
	payload := &payloadData.Data{}
	err := proto.Unmarshal(payloadRaw, payload)
	if err != nil {
